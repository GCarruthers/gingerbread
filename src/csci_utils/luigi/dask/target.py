...
class ParquetTarget(BaseDaskTarget):
    @classmethod
    def _read(cls, path, **kwargs):
        return read_parquet(os.path.join(path, "*.parquet"), **kwargs)

    @classmethod
    def _write(cls, collection, path, **kwargs):
        local_kwargs = kwargs.copy()
        local_kwargs.pop("compute")
        to_parquet(collection, os.path.join(path), compute=True, **local_kwargs)


class CSVTarget(BaseDaskTarget):
    @classmethod
    def _read(cls, path, **kwargs):
        return read_csv(os.path.join(path, "*.csv"), **kwargs)
    ...
    def _write(cls, collection, path, **kwargs):
        ...
        local_kwargs = kwargs.copy()
        local_kwargs.pop("compute")
        to_csv(os.path.join(path), compute=True, **local_kwargs)
